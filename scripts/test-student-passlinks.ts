...
section('Task 4: passlink-admin lib')
{
  ...
  assertEqual(/export async function prefetchAuthUserIdsByEmail\s*\(/.test(s), true, 'prefetchAuthUserIdsByEmail exported')
...
section('Task 5: admin passlink routes')
{
  ...
  assertEqual(/prefetchAuthUserIdsByEmail\(admin\)/.test(issue) && /ensureSubjectAndMint\(admin, s, baseUrl, rotateAll, authMap\)/.test(issue), true, 'issue prefetches auth map (no per-subject listUsers)')
