 ...
 *
 * Functionality matches v1 exactly — same data, same generate /
 * regenerate flow, same copy-to-clipboard — but presentation gets
 * the full v2 treatment: pillar tints per skill card, Apple-HIG
 * layered surfaces, the same depth language as the rest of the v2
 * IA. A more comprehensive Career module is on the roadmap; this
 * is the polished v1-feature-set port until that lands.
 *
 * Empty state in demo: by design /api/student/career returns null
 * for a fresh demo so the "Generate Career Output" CTA is visible.
 * Clicking it hits /api/career/generate (demo-aware: returns the
 * static seed after a brief delay).
 */
...
export function CareerView() {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      // Refetch our wrapper for the pillar-enriched shape rather
      // than trying to reshape /api/career/generate's payload here.
      // The wrapper persists the "generated" state via the demo's
      // post-generate cookie (see /api/student/career).
